...
//	  if ($this->auth->validate($this->userID)) {
		...
		$players = $this->registration->getInventoryPlayers();

//		error_log("Player: " . print_r($player));
		
		$this->vars["players"] = $players;
		$this->vars["barcode"] = $_GET["barcode"];
...
//		error_log("Player: " . print_r($player));
		...
		$invID=$this->registration->rentInventory($playerID, $barcode, $item);
	
		if ($invID) {
			$this->vars["message"] = "Success!";
			$this->vars["alertMessage"] = "alert-success";
		}
		else {
			$this->vars["message"] = "Error!";
			$this->vars["alertMessage"] = "alert-error";
		}
		$this->vars["active"] = "inventory";
